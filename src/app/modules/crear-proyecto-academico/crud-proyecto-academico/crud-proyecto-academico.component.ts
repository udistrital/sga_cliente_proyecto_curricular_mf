...
export class CrudProyectoAcademicoComponent implements OnInit, OnDestroy {
  ...
  registro_califacado_acreditacion: RegistroCalificadoAcreditacion =
    new RegistroCalificadoAcreditacion();
  ...
  dependencia_tipo_dependencia: DependenciaTipoDependencia =
    new DependenciaTipoDependencia();
  ...
  Campo12Control = new FormControl('', [
    Validators.required,
  ...
  constructor(
    ...
    this.subscription = this.listEnfasisService
      .getListEnfasis()
      .subscribe((listEnfasis) => {
        if (listEnfasis) {
  ...
  onCreateEmphasys(event: any) {
    ...
    if (
      !this.arr_enfasis_proyecto.find(
        (enfasis: any) => emphasys.Id === enfasis.Id
      ) &&
      emphasys.Id
    ) {
  ...
  onDeleteEmphasys(event: any) {
    ...
    this.arr_enfasis_proyecto.splice(
      findInArray(event.Id, this.arr_enfasis_proyecto, 'Id'),
      1
    );
    this.dataSource = new MatTableDataSource(this.arr_enfasis_proyecto);
  }
      ...
      .subscribe(
        (res: ApiResponse<any>) => {
          if (res.Success && res.Data.length > 0) {
            ...
            // checks
            this.checkciclos =
              proyecto_a_clonar.ProyectoAcademico.CiclosPropedeuticos;
            this.checkofrece = proyecto_a_clonar.ProyectoAcademico.Oferta;
            // selects
            // unidad de tiempo
            this.opcionSeleccionadoUnidad = this.unidad.find(
              (unidad_temp: any) =>
                unidad_temp.Id ===
                proyecto_a_clonar.ProyectoAcademico.UnidadTiempoId
            );
            this.opcionSeleccionadoNivel = this.nivel.find(
              (nivel_temp: any) =>
                nivel_temp.Id ===
                proyecto_a_clonar.ProyectoAcademico.NivelFormacionId.Id
            );
            this.opcionSeleccionadoMeto = this.metodo.find(
              (metodologia_temp: any) =>
                metodologia_temp.Id ===
                proyecto_a_clonar.ProyectoAcademico.MetodologiaId.Id
            );
            this.opcionSeleccionadoFacultad = this.facultad.find(
              (facultad_temp: any) =>
                facultad_temp.Id === proyecto_a_clonar.IdDependenciaFacultad
            );
            this.opcionSeleccionadoArea = this.area.find(
              (area_temp: any) =>
                area_temp.Id ===
                proyecto_a_clonar.ProyectoAcademico.AreaConocimientoId
            );
            this.opcionSeleccionadoNucleo = this.nucleo.find(
              (nucleo_temp: any) =>
                nucleo_temp.Id ===
                proyecto_a_clonar.ProyectoAcademico.NucleoBaseId
            );
            this.opcionSeleccionadoEspacioFisico = this.espacio_fisico.find(
              (espacio_fisico_temp: any) =>
                espacio_fisico_temp.Id === proyecto_a_clonar.IdEspacioFisico
            );
            // info basica
            this.basicform = this.formBuilder.group({
              ...
              abreviacion_proyecto: [
                proyecto_a_clonar.ProyectoAcademico.CodigoAbreviacion,
                Validators.required,
              ],
              correo_proyecto: [
                proyecto_a_clonar.ProyectoAcademico.CorreoElectronico,
                [Validators.required, Validators.email],
              ],
              numero_proyecto: [
                proyecto_a_clonar.TelefonoDependencia,
                Validators.required,
              ],
              creditos_proyecto: [
                proyecto_a_clonar.ProyectoAcademico.NumeroCreditos,
                [
                  Validators.required,
                  Validators.maxLength(4),
                  Validators.pattern('^[0-9]*$'),
              ...
              duracion_proyecto: [
                proyecto_a_clonar.ProyectoAcademico.Duracion,
                [
                  Validators.required,
                  Validators.maxLength(3),
                  Validators.pattern('^[0-9]*$'),
            ...
            this.compleform = this.formBuilder.group({
              titulacion_snies: [
                proyecto_a_clonar.Titulaciones.find(
                  (titulacion: any) => titulacion.TipoTitulacionId.Id === 1
                ).Nombre,
                Validators.required,
              ],
              titulacion_mujer: [
                proyecto_a_clonar.Titulaciones.find(
                  (titulacion: any) => titulacion.TipoTitulacionId.Id === 3
                ).Nombre,
                Validators.required,
              ],
              titulacion_hombre: [
                proyecto_a_clonar.Titulaciones.find(
                  (titulacion: any) => titulacion.TipoTitulacionId.Id === 2
                ).Nombre,
                Validators.required,
              ],
              competencias: [
                proyecto_a_clonar.ProyectoAcademico.Competencias,
                Validators.required,
              ],
            });
          } else {
            this.snackBar.open(
              this.translate.instant('proyecto.proyecto_no_cargado'),
              '',
  ...
  loadarea() {
    this.parametrosService
      .get(
        'parametro?query=Activo:true,TipoParametroId:4,ParametroPadreId__Id__isnull:true&limit=0'
      )
      .subscribe(
        (res) => {
          const r = <any>res;
  ...
  loadunidadtiempo() {
    this.parametrosService
      .get('parametro?query=TipoParametroId:7,Activo:true&limit=0')
      .subscribe(
        (res) => {
          const r = <any>res;
          if (res !== null && r.Type !== 'error') {
  ...
  loadnucleo(id: number) {
    this.parametrosService
      .get(
        `parametro?query=Activo:true,TipoParametroId:4,ParametroPadreId__Id:${id}&limit=0`
      )
      .subscribe(
        (res) => {
          const r = <any>res;
          ...
          if (this.isObjectEmpty(res.proyecto_snies)) {
            const opt: any = {
              ...
              html: this.translate.instant(
                'proyecto.detalle_proyecto_no_encontrado',
                { codigo_snies: codigo }
              ),
              icon: 'error',
              buttons: true,
            ...
            const opt: any = {
              ...
              html: this.translate.instant(
                'proyecto.detalle_proyecto_encontrado',
                {
                  codigo_snies: codigo,
                  codigo_interno: proyecto.codigo_proyecto,
            ...
            Swal.fire(opt).then((willCreate: any) => {
              if (willCreate.value) {
                this.basicform
                  .get('codigo_interno')
                  .setValue(proyecto.codigo_proyecto);
                this.basicform
                  .get('nombre_proyecto')
                  .setValue(proyecto.nombre_proyecto);
              }
            });
          }
        },
    ...
    try {
      if (
        ...
        this.proyecto_academico = {
          ...
          ProyectoPadreId: this.proyecto_padre_id.Id
            ? this.proyecto_padre_id
            : null,
        };
        ...
        this.registro_califacado_acreditacion = {
          ...
          FechaCreacionActoAdministrativo:
            moment(this.fecha_creacion).format('YYYY-MM-DDTHH:mm') + ':00',
        ...
        const opt: any = {
          ...
          text: this.translate.instant(
            'proyecto.seguro_continuar_registrar_proyecto'
          ),
          icon: 'warning',
          buttons: true,
          dangerMode: true,
        ...
        Swal.fire(opt).then(async (willCreate: any) => {
          if (willCreate.value) {
            ...
            await this.uploadFilesCreacionProyecto([
              this.fileResolucion,
              this.fileActoAdministrativo,
            ]);
            ...
            this.registro_califacado_acreditacion.EnlaceActo =
              this.idDocumentoResolucion + '';
            this.proyecto_academico.EnlaceActoAdministrativo =
              this.idDocumentoAdministrativo + '';
            this.sgaProyectoCurricularMidService
              ...
              .subscribe(
                (res: any) => {
                  if (res.Type === 'error') {
                    ...
                    this.snackBar.open(
                      this.translate.instant('proyecto.proyecto_no_creado'),
                      '',
                      { duration: 3000, panelClass: ['error-snackbar'] }
                    );
